...
#
        ...
        if [ ! -f $output_dir/${roi_name}_chunk_normalized_zscored_${desc}.nii.gz ] || [ $no_checks ]; then
            zscore_chunks $output_dir/${roi_name}_chunk_normalized_${desc}.nii.gz \
             $output_dir/${roi_name}_chunk_normalized_zscored_${desc}.nii.gz \
             $output_dir
        fi
...
while IFS=, read -r subj_id ses_id roi_name desc mri_path roi_path output_dir || [ -n "$output_dir" ]; do
    ...
    echo "$subj_id,$ses_id,$roi_name,$output_dir/${roi_name}_chunk_normalized_zscored_${desc}.nii.gz" >> $R_list
