run_TA_parallel_ep <- function(input, output, bins = 4, threads = 5, save_csv = TRUE) {
...
source("bin/get_TA_stats_single_ep.R")
...
chunk_multiplyer <- 5 # How many subjects to run per thread before saving to CSV?
    ...
    ta_stats_all <- foreach(i=chunk:chunk_end, .combine = "rbind", .inorder = FALSE,
              ...
              # So long as .errorhandling = c("remove") in foreach function arguments. 
                ...
                ta_stats_single <- withTimeout({
                  get_TA_stats_single_ep(input = input_line, bins = bins);
