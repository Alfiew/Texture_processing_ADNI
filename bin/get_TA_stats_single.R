get_TA_stats_single <- function(input, bins = 4) {
...
# Inputs: input: dataframe with 4 elements: Subject ID, Session value, ROI name, ROI path
        # bins: The number of bins that the TA will be run on (relevant only for GLCM and GLRLM and)
...
# Written by Alfie Wearn 2021-08-20. 
  ...
  # 3. All stats
    ...
    ROI <- ROI %>%
      radiomics_all(bins_in = bins, equal_prob = FALSE, geometry_discretized = FALSE)
  ...
  if(2%in%bins){
    b2_glcm_stats <- do.call(cbind, ROI$stat_glcm_mean$es_b2_d1_mean)
    b2_glrlm_stats <- do.call(cbind, ROI$stat_glrlm_mean$es_b2_mean)
  }
  
  if(4%in%bins){
    b4_glcm_stats <- do.call(cbind, ROI$stat_glcm_mean$es_b4_d1_mean)
    b4_glrlm_stats <- do.call(cbind, ROI$stat_glrlm_mean$es_b4_mean)
  }
  
  if(8%in%bins){
    b8_glcm_stats <- do.call(cbind, ROI$stat_glcm_mean$es_b8_d1_mean)
    b8_glrlm_stats <- do.call(cbind, ROI$stat_glrlm_mean$es_b8_mean)
  }
  
  if(16%in%bins){
    b16_glcm_stats <- do.call(cbind, ROI$stat_glcm_mean$es_b16_d1_mean)
    b16_glrlm_stats <- do.call(cbind, ROI$stat_glrlm_mean$es_b16_mean)
  }
  
  if(32%in%bins){
    b32_glcm_stats <- do.call(cbind, ROI$stat_glcm_mean$es_b32_d1_mean)
    b32_glrlm_stats <- do.call(cbind, ROI$stat_glrlm_mean$es_b32_mean)
  }
  
  if(64%in%bins){
    b64_glcm_stats <- do.call(cbind, ROI$stat_glcm_mean$es_b64_d1_mean)
    b64_glrlm_stats <- do.call(cbind, ROI$stat_glrlm_mean$es_b64_mean)
  }
    ...
    glcm_cols <- paste("glcm_", sep="", names(ROI$stat_glcm_mean$es_b4_d1_mean))
    glrlm_cols <- paste("glrlm_", sep ="", names(ROI$stat_glrlm_mean$es_b4_mean))
