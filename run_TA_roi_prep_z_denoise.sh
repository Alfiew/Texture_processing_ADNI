...
source ./bin/denoise_chunks.sh
source ./bin/normalize_chunks.sh
source ./bin/zscore_intensity_scale_chunks.sh
...
#
        ...
        if [ ! -f $output_dir/${roi_name}_chunk_${desc}.nii.gz ] || [ $no_checks ]; then
        ...
        if [ ! -f $output_dir/${roi_name}_chunk_denoised_${desc}.nii.gz ] || [ $no_checks ]; then
            # Denoise those created chunks
            denoise_chunks $output_dir/${roi_name}_chunk_${desc}.nii.gz $output_dir/${roi_name}_chunk_denoised_${desc}.nii.gz
        fi
        
        if [ ! -f $output_dir/${roi_name}_chunk2_denoised_${desc}.nii.gz ] || [ $no_checks ]; then
        # Denoising the chunks creates a soft border around the ROI - so 'extract' it with the ROI mask again
            extract_chunks $output_dir/${roi_name}_chunk_denoised_${desc}.nii.gz $roi_path $output_dir/${roi_name}_chunk2_denoised_${desc}.nii.gz
        fi
        
        if [ ! -f $output_dir/${roi_name}_chunk2_denoised_normalized_${desc}.nii.gz ] || [ $no_checks ]; then
            # Normalize those created chunks using the μ +/- 3σ method
            normalize_chunks $output_dir/${roi_name}_chunk2_denoised_${desc}.nii.gz $output_dir/${roi_name}_chunk2_denoised_normalized_${desc}.nii.gz
        fi
        
        if [ ! -f $output_dir/${roi_name}_chunk2_denoised_normalized_zscored_${desc}.nii.gz ] || [ $no_checks ]; then
            zscore_chunks $output_dir/${roi_name}_chunk2_denoised_normalized_${desc}.nii.gz \
             $output_dir/${roi_name}_chunk2_denoised_normalized_zscored_${desc}.nii.gz \
             $output_dir
        fi
...
while IFS=, read -r subj_id ses_id roi_name desc mri_path roi_path output_dir || [ -n "$output_dir" ]; do
    ...
    echo "$subj_id,$ses_id,$roi_name,$output_dir/${roi_name}_chunk2_denoised_normalized_zscored_${desc}.nii.gz" >> $R_list
